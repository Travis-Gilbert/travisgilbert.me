...
| Scene OS v2 rich renderer (10 stages) | Substrate code complete; **first production caller landed 2026-05-19** | Substrate-and-projection architecture: atoms, transitions, choreographer, glyph atlas, coord-space interp, scene artifacts, composition signals, outcome critic. ADR, runbook, catalog, migration guide in `Index-API/docs/plans/scene-os-rich-renderer-v2/`. Until 2026-05-19 every `ProjectionCapability` instantiation lived in `tests/`; the year-old "no production caller" deferral was reversed with the patent_diagram path. |
| Scene OS v2 first production caller (patent_diagram) | Backend + frontend shipped, behind `SCENE_OS_V2_PATENT_ENABLED=false` flag | 4 commits 2026-05-19: f7d33b87 (backend: catalogs.py + patent_scene resolver + compile_scene_package_v2 + ask_visual_payload integration), f984599d (frontend: PatentDiagramProjection.ts + PatentPlateShell.tsx + _init.ts registration), 123577f9 (docs reversal of phase-c-status). 17 new tests pass; e2e smoke confirms both v1 + v2 paths emit. NOT yet user-visible — needs feature flag enable + VisualRenderer dispatch swap (~20 lines). Status doc: `Index-API/docs/plans/scene-os-patent-diagram/phase-c-status.md`; orchestration report: `orchestration-report-2026-05-19.md`. |
| Encode UI plan tree | Plan complete on disk; build deferred | 9 files in `Index-API/docs/plans/encode-ui/` (README, implementation-plan, visual-register-lock, three-modes-spec, capability-targeting-spec, inference-engine-routing, promotion-gate-uncertainty, blocked-list, orchestration-report-2026-05-18). Three modes (direct training / cross-domain synthesis / self-reflective gap analysis), patent-instrument visual register inherited from Theseus.html + ACT, promotion-state ladder mirrors ACT outcome-calibrated layer. Build sequence in implementation-plan §Rollout. |
| Scene OS takeover wiring (button removal + v1→v2 adapter + SceneHostV2 in portal + radix-colors + GraphForceOrbit/ImageEvidence library substitution + screen-shake H2 fix) | Complete; DEFERRED behind Theseus redesign | Per user 2026-05-15: Scene OS itself is paused until Theseus redesign lands. The takeover plumbing this session built remains in place but the renderer aesthetic (beige paneled CSS) was rejected as "hideous." Next Scene OS work resumes after redesign Phase 6. |
...
| Substrate-native browser (GUI-for-AI plan) | In progress 2026-05-30, multi-repo (theorem.git + Index-API) | The surface where search + SceneOS + the reconstruction engine + Servo converge. Shipped this session: (1) **search-as-graph** in theorem.git: Rust `search_substrate` seam + SERP page + **d3 force-directed graph** + in-page search box + live `serp_server` example (rustyred-web); (2) **reconstruction engine ported whole into Theorem** (`22e8c42`, compiles + 9 tests; standalone crate path-dep'ing civic-atlas-types/theseus-client temporarily, NOT in workspace members so CI stays uncoupled); (3) **engine→SceneOS atom bridge** wired both sides (Theorem `scene_atoms.rs` `6905575` emits; Index-API `atoms.py` `c310e217` ingests; round-trip verified); (4) **SceneOS projection catalog grown** — `heatmap_grid` (Index-API `ac896b70`, now 7 projections); (5) **SceneOS→Theorem port planned** (theorem.git `a249160`, multi-agent). Refs: `Theseus/The Browser.md`, `Downloads/browser-procedural-sceneos-plan.md`, `Theorem/docs/plans/sceneos-to-theorem/`. |

**Next step:** Active track is the **substrate-native browser** (spans theorem.git + Index-API). Two parallel lanes on the **SceneOS→Theorem port** (plan: `Theorem/docs/plans/sceneos-to-theorem/implementation-plan.md`, commit `a249160`), split along the atoms-JSON seam (defined both sides: Theorem `scene_atoms.rs` `6905575` + Index-API `atoms.py` `c310e217`):
- **Codex owns lane A** (Rust scene director, in-process with RustyRed): reimplement the SceneOS backend — atoms + catalogs + `compile` + LIDA `projection_select`/`chrome_select` — in Rust in Theorem. Source to port from: `Index-API/apps/notebook/scene_os/`. Travis's steer: Rust, in-process (not a Python copy).
- **Claude Code owns lane B** (renderer bundle): extract the SceneOS renderer into a self-contained Theorem web bundle Servo serves — the **SERP pattern**, NOT the React stack for slice 1: esbuild bundle of the pure projection adapters (`Theseus-UI/src/scene-os/projections/*.ts`) + a vanilla canvas/d3 renderer that takes a scene-package JSON → runs the chosen projection → draws atoms+relations. The full React `SceneHost`/`AtomSubstrate` is a later enrichment.
- **Lane C** (Codex, plan step 7, last): Servo intercepts a scene URL → lane A produces the package → serve lane B's bundle with it injected (the SERP injection pattern).
Lanes A and B build in parallel across the seam; the reconstruction engine (Theorem) already emits atoms through it. Remaining browser plan steps after: 4 (d3-annotation), 6 (cost-graded dossier), 8 (federated crawl). The coordinate MCP backend was flaky/down (404/503) all session — Codex handoffs ride on commit messages + Travis relaying.

**Parallel/paused track — Substrate Console:** SC-1 backend committed (`107f8b4e`); SC-2 (assistant-ui `useLocalRuntime` + streaming `ChatModelAdapter`, raw `/console/chat` vs substrate `/ask/async`) + SC-3 (`/console` UI) remain. Plan `Index-API/docs/plans/substrate-console/implementation-plan.md`. Resume if the browser track pauses.
...
Track B (Scene OS v2 cutover — from 2026-05-19 orchestration): Make the v2 patent_diagram path user-visible. (1) Set `SCENE_OS_V2_PATENT_ENABLED=true` on the Railway worker, restart, curl-verify a patent-style query returns `structured_visual.scene_package_v2`. (2) Wire `<VisualRenderer>` in threads-v2 to dispatch v2 packages to `<SceneHostV2>` instead of v1 `<PatentDiagram>` when `scene_package_v2` is present (~20 lines). (3) Optionally productize the harness MCP separately from theseus-mcp (split `mcp_server/tools/{context_tools,harness_tools,agent_gateway_tools,verbs}.py` into a `theorem-mcp` deployable while keeping `theseus-mcp` as the internal kitchen-sink). Plan: `Index-API/docs/plans/scene-os-patent-diagram/orchestration-report-2026-05-19.md`.
...
- **Scene OS v2 follow-ups (from 2026-05-19 orchestration):**
  - Enable `SCENE_OS_V2_PATENT_ENABLED=true` on Railway worker; curl-verify
  - Wire `<VisualRenderer>` v2 dispatch (~20 lines) to mount `<SceneHostV2>` when `structured_visual.scene_package_v2` present
  - Productize harness MCP separately from theseus-mcp (the `context_tools.py` + `harness_tools.py` + `agent_gateway_tools.py` + `verbs.py` subset)
  - Add `critique_memory(target_actor, since?)` MCP verb for cross-vendor critique pattern (adversarial collaboration between Claude Code and Codex memories — strongest insight from the 2026-05-19 meta-session)
- **Modal billing + RunPod migration (unresolved, blocks 26B quality):**
  - Modal workspace hit spend cap; every 26B request returns 429. Patent_diagram falls back to 4B with `degraded:true` correctly surfaced. Resolution is personal-billing track.
  - RunPod cutover for theseus-gemma-26b-role, theseus-gemma-26b-v2, theseus-gemma4b-gguf, theseus-e4b-agent-sft is the architectural answer; `apps/notebook/speaking_dispatch.py:94` already reads `RUNPOD_26B_URL` so the cutover is env-var + deploy, no code change.
- **claude.ai integration (planning only):** Anthropic shipped both a first-party Memory feature (auto-load `/memories` directory equivalent for claude.ai) AND a Skills feature that works in claude.ai. The cleanest path is: harness MCP exposed as a remote MCP connector + a "Theseus Memory" Skill with YAML frontmatter that auto-triggers + let Anthropic's Memory be the working set with the harness as cold storage. Productize harness MCP first so it has a stable external surface.
- **Deployment check:** Travis reported `theseus.travisgilbert.me` is stuck on Railway as of 2026-05-19. Next session should check Railway worker + web service status before doing anything else.

## Recent Decisions

| Decision | Choice | Why |
|----------|--------|-----|
| Port a Rust engine WHOLE via temporary path-deps, not genericized (2026-05-30) | When Travis said "port the entire engine, building primitives aren't hurting, strip later," abandoned a 448-error genericization for a clean whole-port: copy pristine `civic-atlas-reconstruction-engine` into `Theorem/rustyredcore_THG/crates/reconstruction-engine` + path-dep `civic-atlas-types`/`theseus-client` (temporary, building primitives intact). 9 tests pass. Strip-to-self-standing = a planned focused session (crate README). | Genericizing a 3713-line engine whose "generic" types embed civic types (CivicObject/ReconstructionSource/TenantContext) is delicate surgery; a whole-port compiles in 34s, and the two copies are meant to diverge ("they do different things"), same as buildings staying in civic. |
| SceneOS → Theorem splits along the atoms-JSON seam (2026-05-30) | The substrate-native browser plan spans 2 repos: theorem.git (Rust browser + engine + engine-side SceneOS adapter) and Index-API/Theseus.git (SceneOS Python backend + TS renderer). Porting SceneOS to Theorem splits on the atoms-JSON contract: Codex = Rust scene director (lane A, in-process), Claude Code = renderer web bundle (lane B, SERP pattern). | The atoms-JSON contract (`6905575` + `c310e217`) lets director (Rust, Codex) and renderer (web, me) be written in different languages by different agents in parallel and still interoperate. Reminder: not all browser work is in theorem.git — SceneOS lives in Index-API until lanes A/B port it. |
| Cross-repo-coupled crate stays OUT of the parent workspace members (2026-05-30) | A crate that path-deps another repo (`reconstruction-engine` → `civic-atlas-types`) gets its own standalone `[workspace]` table and is NOT added to the parent `rustyredcore_THG` members. Build it directly in its dir (`cargo test`). | Keeps the parent build + CI uncoupled from the other repo's presence: a fresh clone / CI without the sibling repo still builds, because the workspace never tries to build the path-dep'd crate. Opt-in, built only when explicitly targeted. |
...
| Scene OS v2 deferral was circular, reversed 2026-05-19 | Treat "no production caller" as the gap THIS session closes, not a precondition. Catalogs/resolvers/compile entry live in `apps/notebook/scene_os/{catalogs,resolvers/patent_scene,compile/v2_compile}.py`; frontend in `Theseus-UI/src/scene-os/{projections/PatentDiagramProjection,chrome/PatentPlateShell,_init}`. | A year of sessions inherited the prior session's "no caller, defer" framing and treated it as architectural truth. The deferral was risk-aversion smuggled in as architecture. Closed by shipping the smallest end-to-end v2 path (patent_diagram) behind a feature flag so the v1 path remains zero-risk fallback. Pattern lesson: when a gap-name persists across sessions, the gap-naming has become the close, not the open. |
| Cross-vendor critique = adversarial collaboration pattern | When Claude critiques Codex's memory and vice versa, the different vendor priors break the bias problem that same-model reflective passes have. Formalized by Kahneman + Klein in behavioral econ literature. | Same-model reflection confirms priors. Cross-model critique surfaces blind spots structurally. Travis named this in the 2026-05-19 meta-session; queued as Scene OS v2/agent-memory v1.5 priority. Implementation rides on existing mention/orchestrate infrastructure: one MCP verb `critique_memory(target, since?)` + one document type + existing mention flow. |
| Engineering-partner mode, not yes-and mode | When proposing smaller scope, surface the risk BEFORE the proposal ("I'm worried about X; scope down or push through?") instead of after. When the user asks "what's the risk you're worried about?", answer directly. Default to "ship now and revert if wrong" when scope is contested. | 2026-05-19 meta-conversation observation: I had been smuggling risk-aversion into technical recommendations as architectural caution. The "v2 has no production caller" framing was the cleanest example — held for a month. Naming the hidden discount upfront converts a 30-minute negotiation into a 30-second one. New tools don't fix this; following the existing rules does. |
| MCP server split: three tiers | Theorem MCP (Python FastMCP, action/harness verbs) + Theseus MCP (Python FastMCP, engine/diagnostic) + RustyRed-THG MCP (native Rust, graph reads/algorithms). Theorem MCP at `mcp_server_theorem/`, Theseus stays at `mcp_server/`. Pure graph reads bypass Python entirely via `mcp__rustyred-thg__*`. | Single 105-tool Python MCP was an undifferentiated surface. Splitting by product line lets each server expose a coherent toolset. RustyRed-THG's native MCP delivers Rust-native latency for graph queries (vs Python→HTTP→Django→THG client chain). See `docs/plans/mcp-server-consolidation/`. |
| Form-B naming: drop product prefix entirely | Tools register as `code_explain` not `theseus_code_explain`, `harness_begin` not `theorem_harness_begin`. MCP framework already exposes tools as `mcp__<server>__<tool>` so encoding product in tool name is redundant. Matches Claude Code's built-in convention (Read/Edit/Write — no prefix). | Cleaner for agents — they scan subject (code/harness/document) not product. Saves tokens. Window closes once external users adopt prefixed names. Pre-launch is the right time. |
| Memgraph→RustyRed write-through via post-commit hook (Option I) | New `_dispatch_thg_write_through` in `apps/notebook/graph_writes.py` fires after every Memgraph-primary write commit. Failure is logged WARN and swallowed; never re-raised. Gated by `GRAPH_WRITE_THROUGH_THG_ENABLED` env var (default off until validated). | Lowest staleness (vs scheduled sync). Matches existing `transaction.on_commit(_create_event)` pattern. Easiest to revert (flip env var off). |
| Native dispatch startup probe | `apps/notebook/apps.py:ready()` imports `theseus_native` at app boot and logs INFO (active) or WARNING (fallback to Python push_ppr). | Without this probe, `apps/notebook/sparse_ppr.py`'s silent Python fallback was invisible in production. PT-004 surfaced a year-old defect (wrong PyInit symbol in the maturin wheel) the moment the probe shipped. Observability of the slow path firing IS the architectural unlock. |
| PyO3 wheel symbol name override | `rustyredcore_THG/src/lib.rs` uses `#[pyo3(name = "theseus_native")]` on the pymodule fn so the Rust function name (`rustyredcore_THG`, intentional protocol marker) is decoupled from the Python-visible module name (`theseus_native`, what `pyproject.toml` exports). | Lets the Rust crate keep its `_THG` suffix (protocol-family marker per `lib.rs` preamble) while Python imports the canonical `theseus_native`. Symbol mismatch was the year-old silent bug. |
| Railway env-var name discipline | RustyRed-THG config reads `RUSTY_RED_API_TOKENS` / `RUSTYRED_THG_API_TOKENS` — NOT `THG_API_TOKENS` (dead name from before rename). Same for `RUSTY_RED_ALLOWED_ORIGINS`. Setting the wrong env-var name results in empty token list → all bearers get 403. | Cascading auth failures during PT-104 verification surfaced this. Worth a sweep through any service config that references the old `THG_*` prefix; the rename to `RUSTYRED_THG_*` is the canonical form. |
| Re-registration pattern for dual MCP cutover | New `mcp_server_theorem/tools/*.py` imports handlers from `mcp_server/tools/*.py` and calls `mcp.tool(name='short_name')(_handler)` on the new FastMCP instance. Both servers register the same Python function under different names. | Avoids code duplication during the Phase 2/3 cutover window. Old surface keeps working; new surface adds the Form-B names. Phase 3 PT-301 drops the old wrappers after one release. |
